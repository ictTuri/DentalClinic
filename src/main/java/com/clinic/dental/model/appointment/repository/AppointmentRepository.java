...
public interface AppointmentRepository extends JpaRepository<AppointmentEntity, Long>{
	...
	@Query(value = "SELECT * FROM appointments WHERE appointment_id = ?1 AND status NOT IN ('DONE','ACTIVE','USER_CANCELLED','DOCTOR_CANCELLED') AND (AGE(date,now())) < '1' ", nativeQuery = true)
